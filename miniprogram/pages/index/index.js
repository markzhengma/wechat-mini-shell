...
Page({
    ...
    wx.request({
      ...
      success: (res) => {
        ...
        if (res.data.code !== 200) {
          if (res.data.code === 401) {
            console.log(
              "wechat not bind to user, try to get userData with phone number"
            );
  ...
  setLocation: function () {
    ...
    this.setData({
      newUserInput: {
        ...
        location:
          locationIndex !== -1 ? this.data.locationNameList[locationIndex] : "",
      },
    });
  },
  ...
  createUser: function () {
    ...
    let locationIndex = this.data.locationNameList.indexOf(
    ...
    if (!newUserInfo.user_name.match(REGEX_CHINESE)) {
      ...
      (newUserInfo.phone.length !== 7 && newUserInfo.phone.length !== 11) ||
      !newUserInfo.phone.match(/^\d+$/)
    ) {
      Dialog.alert({
        title: "出错了",
      ...
      (newUserInfo.plate.length !== 7 && newUserInfo.plate.length !== 8) ||
      !newUserInfo.plate.match(REGEX_CHINESE)
    ) {
      Dialog.alert({
        title: "出错了",
  ...
  confirmUserCreate: function (locationChar, newUserInfo) {
    ...
    wx.request({
      ...
      data: {
        user_name:
          newUserInfo.user_name.length === 1
            ? newUserInfo.user_name + "先生/女士"
            ...
            if (Array.isArray(findUserList) && findUserList.length === 1) {
              Dialog.alert({
                ...
                wx.navigateTo({
                  url: "/pages/Admin/Admin",
                  success: (res) => {
                    res.eventChannel.emit("adminFindUserInfo", {
                      data: findUserList[0],
                    });
                  },
                });
  ...
  manualSearchSubmit: function () {
    ...
    if (
      ...
      (phone.length !== 7 && phone.length !== 11) ||
      !phone.match(/^\d+$/)
    ) {
      Dialog.alert({
        title: "出错了",
          ...
          Dialog.confirm({
            ...
            .then(() => {
              ...
              wx.navigateTo({
                ...
                success: (res) => {
                  res.eventChannel.emit("manualSearchUser", {
                    data: manualSearchUserData,
                  });
                },
              });
            })
          ...
          bannerList: [
            {
              ...
              thumb_url:
                "https://mmbiz.qpic.cn/mmbiz_png/AAfx4P2w70UHvMicQDl01FXBY31g87z2G5lhxD3W6sL4suP2VK8bXO19RJxcfRqG8e9g4yJhCs7Qq0jE6FGa21A/640?wx_fmt=png&from=appmsg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1",
            },
            {
              title: "劲涌开赛，不息向前！",
              ...
              thumb_url:
                "https://mmbiz.qpic.cn/mmbiz_png/AAfx4P2w70U0CWEqwz3EdWicrovcw1vTIWhUZJF2dhGA00G1yxmBLIAYgZc1iaRuAFeEajH7Fj7BtcQ6ltcp2pCw/640?wx_fmt=png&from=appmsg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1",
            },
          ],
        });
      });
